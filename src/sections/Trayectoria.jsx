...
const hitos = [
  {
    ...
    // Reemplaza imagen con la ruta de tu foto real, ej: "/images/2008-fundacion.jpg"
    imagen: null,
...
function HitoItem({ hito, index }) {
  ...
  return (
    <div className={`relative flex items-start gap-0 ${esIzquierda ? "md:flex-row" : "md:flex-row-reverse"}`}>

      {/* Dot central en la línea */}
      <div className="absolute left-1/2 -translate-x-1/2 top-4 z-10 flex flex-col items-center">
        <div
          className="w-5 h-5 rounded-full border-4 border-white shadow-md transition-transform duration-300"
          style={{ backgroundColor: hito.color }}
        />
      </div>

      {/* Tarjeta — ocupa mitad del ancho */}
      <motion.div
      ...
      >
        ...
        >
          <div
            className="flex items-center justify-between gap-3 rounded-2xl px-5 py-4 shadow-sm border border-white hover:shadow-md transition-all duration-200 bg-white"
          >
            {/* Año + título */}
        ...
        {/* Panel expandible con descripción + placeholder imagen */}
        <AnimatePresence initial={false}>
          {abierto && (
            ...
            >
              <div className="mt-2 rounded-2xl border border-white bg-white shadow-md overflow-hidden">
                {/* Placeholder de imagen — reemplaza hito.imagen cuando tengas tu foto */}
                <div
                ...
                >
                  {hito.imagen ? (
                    ...
                    <span className="opacity-80">Aquí se agregaran las imagenes</span>
                  )}
                </div>
                {/* Descripción */}
    ...
    <section id="trayectoria" className="py-20 bg-white">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="relative">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-[var(--color-primary)]/20 via-[var(--color-primary)]/40 to-[var(--color-primary)]/20" />
